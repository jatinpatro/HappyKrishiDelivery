...
# Sync static pages (privacy policy, terms — live alongside the Flutter app)
if [ -d "$(dirname "$0")/static" ]; then
  rsync -az -e "ssh ${SSH_OPTS}" "$(dirname "$0")/static/" ${REMOTE_USER}@${REMOTE_HOST}:${WEB_DIR}/
  log "✓ Static pages synced"
fi
